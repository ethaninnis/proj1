...
int main() {
    ...
    char SubstitutionKey[] = "ABCDEFGHIJKLMNOPQRTSUVWXYZ"; //hardcoded substitution key
    char text[] = "ABCDE";
...
void DecryptSubstitution(char key[], char text[]) {
    ...
    while (text[i] != '\0') {
        if (text[i] >= 65 && text[i] <= 90) {
            for (PositionNumber = 0; text[i] != key[PositionNumber]; PositionNumber++) { //counting method to find what letter in the substitution key is equal to the letter being substituted
            }
            printf("%c", Alphabet[PositionNumber]);
        } else if (text[i] >= 97 && text[i] <= 122) { //decrypts lowercase text
            text[i] = text[i] - 32; {
                for (PositionNumber = 0; text[i] != key[PositionNumber]; PositionNumber++) {
                }
                printf("%c", Alphabet[PositionNumber]);
            }
